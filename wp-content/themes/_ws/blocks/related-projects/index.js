  ...
  args: {
    ...
    edit: (props) => {
      ...
      return (
        <>
          <InspectorControls>
            <PanelBody title={__("Latest & Upcoming Options", "_ws")}>
              <PostTypeControl
                label={__("Post Type", "_ws")}
                help={__(
                  'Post types with start dates will ordered by "upcoming" instead of "latest"',
                  "_ws"
                )}
                onChange={(newValue) => {
                  ...
                  if (tax.terms.length > 0) {
                    return (
                      <CheckboxGroupControl
                        ...
                        onChange={(newValue) =>
                          setAttributes({
                            taxTerms: { ...taxTerms, [tax.slug]: newValue },
                          })
                        }
